...
void* XT_mul_err_r(void *arg) {
    int id = (int)arg;
    int start_row = id / 5 * blk_r, start_column = id % 5 * blk_c;
    int end_row = min(start_row + blk_r, PICSZ), end_column = start_column + blk_c;
...
void XT_mul_err() {
    ...
    blk_r = PICSZ / thread_n * 5;
    if ( thread_n < 5 ) {
        blk_r = PICSZ;
        blk_c = K / thread_n + !!(K % thread_n);
    }
...
void* X_mul_W_r(void *arg) {
    int id = (int)arg;
    int start_row = id / 5 * blk_r, start_column = id % 5 * blk_c;
    int end_row = min(start_row + blk_r, TRAIN_N), end_column = start_column + blk_c;
    for ( int i = start_row; i < end_row; ++i)
        for ( int j = start_column; j < end_column; ++j)
...
void* Xtest_mul_W_r(void *arg) {
    int id = (int)arg;
    int start_row = id / 5 * blk_r, start_column = id % 5 * blk_c;
    int end_row = min(start_row + blk_r, TEST_N), end_column = start_column + blk_c;
    for ( int i = start_row; i < end_row; ++i)
        for ( int j = start_column; j < end_column; ++j)
            for ( int k = 0; k < PICSZ; ++k)
